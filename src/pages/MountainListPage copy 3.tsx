...
const MountainListPage = () => {
  ...
  /** 필터링 관련 상태 */
  const [filteredList, setFilteredList] = useState<Mountain[]>([]);
  ...
  useEffect(() => {
    const fetchMountains = async () => {
      try {
        setLoading(true);
        const firstQuery = query(collection(db, "mountains"), limit(30));
        const querySnapshot = await getDocs(firstQuery);
        const mountainsData: Mountain[] = querySnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        })) as Mountain[];
        setMountainList(mountainsData);

        // 마지막 문서를 저장하여 페이지네이션에 활용
  ...
  const loadMoreMountains = async () => {
    ...
    try {
      ...
      const mountainsData: Mountain[] = nextSnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      })) as Mountain[];

      setMountainList((prev) => [...prev, ...mountainsData]);
      setLastVisible(nextSnapshot.docs[nextSnapshot.docs.length - 1]);
    } catch (error) {
      console.error("Error loading more mountains:", error);
    } finally {
      setLoading(false);
    }
  };

  // 무한 스크롤 트리거 (마지막 요소가 화면에 보이면 실행)
  useEffect(() => {
    if (inView) {
      loadMoreMountains();
    }
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredList.length > 0 ? (
            filteredList.map((mountain, index) => (
              <div
                key={mountain.id}
              ...
              >
                <div className="h-48 overflow-hidden ">
                  <img
                    src={`https://songtak.github.io/bac_mt_course/assets/bac_img/empty_thumbnail_1.jpeg`}
                    alt={mountain.name}
